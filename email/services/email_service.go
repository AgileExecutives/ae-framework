...
func (e *EmailService) sendSMTPTLS(addr string, auth smtp.Auth, from string, to []string, msg []byte) error {
    ...
    if err != nil { return fmt.Errorf("failed to connect to SMTP server: %v", err) }
    defer conn.Close()

    client, err := smtp.NewClient(conn, tlsConfig.ServerName)
    if err != nil { return fmt.Errorf("failed to create SMTP client: %v", err) }
    defer client.Quit()

    if auth != nil { if err = client.Auth(auth); err != nil { return fmt.Errorf("SMTP authentication failed: %v", err) } }
    if err = client.Mail(from); err != nil { return fmt.Errorf("failed to set sender: %v", err) }
    for _, addr := range to { if err = client.Rcpt(addr); err != nil { return fmt.Errorf("failed to set recipient %s: %v", addr, err) } }
    writer, err := client.Data()
    if err != nil { return fmt.Errorf("failed to get data writer: %v", err) }
    _, err = writer.Write(msg)
    if err != nil { return fmt.Errorf("failed to write message: %v", err) }
    err = writer.Close()
    if err != nil { return fmt.Errorf("failed to close writer: %v", err) }
    return nil
}
...
func (e *EmailService) sendSMTPSTARTTLS(addr string, auth smtp.Auth, from string, to []string, msg []byte) error {
    tlsEnabled := strings.ToLower(utils.GetEnv("SMTP_TLS_ENABLED", "true")) == "true"
    if !tlsEnabled { return smtp.SendMail(addr, auth, from, to, msg) }
    ...
    if err = client.StartTLS(tlsConfig); err != nil { return fmt.Errorf("failed to start TLS: %v", err) }
    if auth != nil { if err = client.Auth(auth); err != nil { return fmt.Errorf("SMTP authentication failed: %v", err) } }
    if err = client.Mail(from); err != nil { return fmt.Errorf("failed to set sender: %v", err) }
    for _, addr := range to { if err = client.Rcpt(addr); err != nil { return fmt.Errorf("failed to set recipient %s: %v", addr, err) } }
    writer, err := client.Data()
...
func (e *EmailService) detectEmailType(subject, htmlBody string) string {
    ...
    if strings.Contains(subjectLower, "verification") || strings.Contains(bodyLower, "verify") { return "📧 EMAIL VERIFICATION" } else if strings.Contains(subjectLower, "reset") || strings.Contains(bodyLower, "reset") { return "🔑 PASSWORD RESET" } else if strings.Contains(subjectLower, "welcome") || strings.Contains(bodyLower, "welcome") { return "👋 WELCOME EMAIL" } else if strings.Contains(subjectLower, "invoice") || strings.Contains(bodyLower, "invoice") { return "🧾 INVOICE" } else if strings.Contains(subjectLower, "appointment") || strings.Contains(bodyLower, "appointment") { return "📅 APPOINTMENT" } else if strings.Contains(subjectLower, "contact") || strings.Contains(bodyLower, "contact form") { return "📝 CONTACT FORM SUBMISSION" } else if strings.Contains(subjectLower, "notification") { return "🔔 NOTIFICATION" } return "📧 GENERAL EMAIL" }
...
func (e *EmailService) displayEmailTypeInfo(emailType, htmlBody string) {
    ...
    case strings.Contains(emailType, "VERIFICATION"):
        ...
        if link != "" { fmt.Printf("🔗 VERIFICATION LINK: %s\n", link) }
    ...
    case strings.Contains(emailType, "PASSWORD"):
        ...
        if link != "" { fmt.Printf("🔗 RESET LINK: %s\n", link) }
...
func (e *EmailService) composeMessage(to, subject, htmlBody, textBody string) string {
    ...
    var from string
    if fromName != "" { from = fmt.Sprintf("%s <%s>", fromName, fromEmail) } else { from = fromEmail }
...
func (e *EmailService) initMockEmailsFile() {
    ...
    if err := os.MkdirAll(dir, 0755); err != nil { log.Printf("Failed to create tmp directory: %v", err); return }
    emptyArray := []MockEmailRecord{}
    data, err := json.MarshalIndent(emptyArray, "", "  ")
    if err != nil { log.Printf("Failed to marshal empty email array: %v", err); return }
    if err := os.WriteFile(mockEmailsFilePath, data, 0644); err != nil { log.Printf("Failed to initialize mock emails file: %v", err); return }
...
func (e *EmailService) saveMockEmail(to, subject, htmlBody, textBody string) {
    ...
    if err != nil { log.Printf("⚠️ Failed to read mock emails file: %v", err); emails = []MockEmailRecord{} } else { if err := json.Unmarshal(data, &emails); err != nil { log.Printf("⚠️ Failed to unmarshal mock emails: %v", err); emails = []MockEmailRecord{} } }
    ...
    if err != nil { log.Printf("❌ Failed to marshal emails: %v", err); return }
    if err := os.WriteFile(mockEmailsFilePath, data, 0644); err != nil { log.Printf("❌ Failed to write mock emails file: %v", err); return }
...
func (e *EmailService) GetLatestEmails() ([]MockEmailRecord, error) {
    ...
    data, err := os.ReadFile(mockEmailsFilePath)
    if err != nil {
        if os.IsNotExist(err) { return []MockEmailRecord{}, nil }
        return nil, fmt.Errorf("failed to read mock emails file: %w", err)
    }
    if err := json.Unmarshal(data, &emails); err != nil { return nil, fmt.Errorf("failed to unmarshal mock emails: %w", err) }
    return emails, nil
}
