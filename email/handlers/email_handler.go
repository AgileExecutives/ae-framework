...
import (
    ...
    _ "github.com/ae/base-server/modules/user/models"
...
func (h *EmailHandler) GetMiddleware() []gin.HandlerFunc { return []gin.HandlerFunc{middleware.AuthMiddleware(h.db)} }
...
func (h *EmailHandler) GetEmails(c *gin.Context) {
    ...
    for _, email := range emails { responses = append(responses, email.ToResponse()) }
...
func (h *EmailHandler) GetEmail(c *gin.Context) {
    ...
    if err != nil { c.JSON(http.StatusBadRequest, models.ErrorResponseFunc("Invalid email ID", err.Error())); return }

    var email models.Email
    if err := h.db.First(&email, id).Error; err != nil {
        if err == gorm.ErrRecordNotFound { c.JSON(http.StatusNotFound, models.ErrorResponseFunc("Email not found", "Email with specified ID does not exist")); return }
        c.JSON(http.StatusInternalServerError, models.ErrorResponseFunc("Failed to retrieve email", err.Error())); return
    }
...
func (h *EmailHandler) SendEmail(c *gin.Context) {
    ...
    if err := c.ShouldBindJSON(&req); err != nil { c.JSON(http.StatusBadRequest, models.ErrorResponseFunc("Invalid request", err.Error())); return }

    email := models.Email{To: req.To, From: req.From, Subject: req.Subject, Body: req.Body, HTMLBody: req.HTMLBody, Status: "pending"}
    if err := h.db.Create(&email).Error; err != nil { c.JSON(http.StatusInternalServerError, models.ErrorResponseFunc("Failed to create email", err.Error())); return }

    go func() {
        textBody := req.Body
        if textBody == "" { textBody = req.Subject }
        err := h.emailService.SendEmail(req.To, req.Subject, req.HTMLBody, textBody)
        status := "sent"; var errorMessage string
        if err != nil { status = "failed"; errorMessage = err.Error() }
...
func (h *EmailHandler) GetEmailStats(c *gin.Context) {
    type EmailStats struct{ Total, Pending, Sent, Delivered, Failed int64 }
