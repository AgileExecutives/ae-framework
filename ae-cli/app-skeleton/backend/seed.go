...
func RunIfEmpty(db *gorm.DB) error {
	var userCount int64
	if err := db.Model(&pkgmodels.User{}).Count(&userCount).Error; err != nil {
...
func seedTenant(db *gorm.DB, customerID uint) (pkgmodels.Tenant, error) {
	...
	if err := db.Create(&tenant).Error; err != nil {
		return pkgmodels.Tenant{}, fmt.Errorf("failed to create server-test tenant: %w", err)
	}
	return tenant, nil
}
...
func seedAdminUser(db *gorm.DB, tenantID, organizationID uint) (pkgmodels.User, string, error) {
	...
	if err != nil {
		return pkgmodels.User{}, "", fmt.Errorf("failed to hash server-test admin password: %w", err)
	}
	now := time.Now()
	user := pkgmodels.User{
		Username:        username,
	...
	if err := db.Create(&user).Error; err != nil {
		return pkgmodels.User{}, "", fmt.Errorf("failed to create server-test admin user: %w", err)
	}
