...
func TestTemplateRenderingFlows(t *testing.T) {
	...
	if status != 200 {
		t.Fatalf("booking render status=%d body=%s", status, string(body))
	}
	...
	if status != 200 {
		t.Fatalf("password render status=%d body=%s", status, string(body))
	}
	...
	if status != 200 {
		t.Fatalf("invoice render status=%d body=%s", status, string(body))
	}
	...
	if status != 400 {
		if status != 422 {
			t.Fatalf("expected 400/422 for missing data, got %d body=%s", status, string(body))
		}
	}
	...
	if status != 404 {
		t.Fatalf("expected 404 for invalid template key, got %d body=%s", status, string(body))
	}
	...
	if !(status == 404 || status == 400) {
		t.Fatalf("expected 404 or 400 for wrong channel, got %d body=%s", status, string(body))
	}
	...
	if status != 200 {
		t.Fatalf("extra data render status=%d body=%s", status, string(body))
	}
	if !strings.Contains(string(body), "Test User") {
		t.Fatalf("extra render missing expected content: %s", string(body))
	}
}
