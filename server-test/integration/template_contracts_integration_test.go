...
func TestTemplateContractsEndpoints(t *testing.T) {
    ...
    if err != nil { t.Fatalf("list contracts failed: %v", err) }
    if resp.StatusCode != 200 { t.Fatalf("list contracts status=%d body=%s", resp.StatusCode, string(body)) }

    // Get welcome contract
    resp, body, err = authReq("GET", "http://localhost:8080/api/v1/templates/contracts/welcome", nil)
    if err != nil { t.Fatalf("get welcome contract failed: %v", err) }
    if resp.StatusCode != 200 { t.Fatalf("welcome contract status=%d body=%s", resp.StatusCode, string(body)) }
    var parsed map[string]interface{}
    if err := json.Unmarshal(body, &parsed); err != nil { t.Fatalf("invalid welcome response: %v", err) }
    data := parsed["data"].(map[string]interface{})
    if data["template_key"] != "welcome" { t.Fatalf("welcome key mismatch: %v", data["template_key"]) }
    ...
    if err != nil { t.Fatalf("get booking contract failed: %v", err) }
    if resp.StatusCode != 200 { t.Fatalf("booking contract status=%d body=%s", resp.StatusCode, string(body)) }

    // Get password_reset contract
    resp, body, err = authReq("GET", "http://localhost:8080/api/v1/templates/contracts/password_reset", nil)
    if err != nil { t.Fatalf("get password reset contract failed: %v", err) }
    if resp.StatusCode != 200 { t.Fatalf("password reset contract status=%d body=%s", resp.StatusCode, string(body)) }

    // Get invoice contract
    resp, body, err = authReq("GET", "http://localhost:8080/api/v1/templates/contracts/invoice", nil)
    if err != nil { t.Fatalf("get invoice contract failed: %v", err) }
    if resp.StatusCode != 200 { t.Fatalf("invoice contract status=%d body=%s", resp.StatusCode, string(body)) }
    ...
    if err != nil { t.Fatalf("get welcome sample failed: %v", err) }
    if resp.StatusCode != 200 { t.Fatalf("welcome sample status=%d body=%s", resp.StatusCode, string(body)) }
    ...
    if err != nil { t.Fatalf("validate welcome failed: %v", err) }
    if resp.StatusCode != 200 { t.Fatalf("validate welcome status=%d body=%s", resp.StatusCode, string(body)) }
    var vresp map[string]interface{}
    if err := json.Unmarshal(body, &vresp); err != nil { t.Fatalf("invalid validate response: %v", err) }
    vdata := vresp["data"].(map[string]interface{})
    if validRes, ok := vdata["valid"].(bool); !ok || !validRes { t.Fatalf("expected valid=true, got %v", vdata["valid"]) }
    ...
    if err != nil { t.Fatalf("validate welcome invalid failed: %v", err) }
    if resp.StatusCode != 200 { t.Fatalf("validate welcome invalid status=%d body=%s", resp.StatusCode, string(body)) }
    if err := json.Unmarshal(body, &vresp); err != nil { t.Fatalf("invalid validate response 2: %v", err) }
    vdata = vresp["data"].(map[string]interface{})
    if validRes, ok := vdata["valid"].(bool); !ok || validRes { t.Fatalf("expected valid=false, got %v", vdata["valid"]) }
    ...
    if err != nil { t.Fatalf("invoice validate failed: %v", err) }
    if resp.StatusCode != 200 { t.Fatalf("invoice validate status=%d body=%s", resp.StatusCode, string(body)) }
    if err := json.Unmarshal(body, &vresp); err != nil { t.Fatalf("invalid invoice validate response: %v", err) }
    vdata = vresp["data"].(map[string]interface{})
    if validRes, ok := vdata["valid"].(bool); !ok || !validRes { t.Fatalf("expected invoice valid=true, got %v", vdata["valid"]) }
    ...
    if err != nil { t.Fatalf("get non-existent contract failed: %v", err) }
    if resp.StatusCode != 404 { t.Fatalf("expected 404 for non-existent contract, got %d body=%s", resp.StatusCode, string(body)) }
}
