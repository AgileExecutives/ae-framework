...
func TestTemplateCRUD(t *testing.T) {
    ...
    if resp.StatusCode != 200 { t.Fatalf("get templates status=%d body=%s", resp.StatusCode, string(body)) }
    ...
    if resp.StatusCode != 200 { t.Fatalf("get email tpl status=%d", resp.StatusCode) }
    ...
    if resp.StatusCode != 200 { t.Fatalf("update tpl status=%d", resp.StatusCode) }
    ...
    if resp.StatusCode != 200 { t.Fatalf("render updated tpl status=%d body=%s", resp.StatusCode, string(body)) }
    if !strings.Contains(string(body), "John Doe") { t.Fatalf("render content missing: %s", string(body)) }
    ...
    if resp.StatusCode != 201 { t.Fatalf("duplicate tpl status=%d body=%s", resp.StatusCode, string(body)) }
    var created map[string]interface{}
    if err := json.Unmarshal(body, &created); err != nil { t.Fatalf("invalid duplicate response: %v", err) }
    ...
    if resp.StatusCode != 200 { t.Fatalf("deactivate status=%d", resp.StatusCode) }

    status["is_active"] = true
    sb, _ = json.Marshal(status)
    resp, _ = AuthRequest(t, token, "PUT", fmt.Sprintf("http://localhost:8080/api/v1/templates/%d", emailID), sb)
    if resp.StatusCode != 200 { t.Fatalf("reactivate status=%d", resp.StatusCode) }
    ...
    if resp.StatusCode != 200 { t.Fatalf("filter by type status=%d", resp.StatusCode) }

    resp, _ = AuthRequest(t, token, "GET", "http://localhost:8080/api/v1/templates?channel=EMAIL", nil)
    if resp.StatusCode != 200 { t.Fatalf("filter by channel status=%d", resp.StatusCode) }

    // Cleanup deletes
    resp, _ = AuthRequest(t, token, "DELETE", fmt.Sprintf("http://localhost:8080/api/v1/templates/%d", dupID), nil)
    if resp.StatusCode != 204 { t.Fatalf("delete dup status=%d", resp.StatusCode) }

    resp, _ = AuthRequest(t, token, "DELETE", fmt.Sprintf("http://localhost:8080/api/v1/templates/%d", pdfID), nil)
    if resp.StatusCode != 204 { t.Fatalf("delete pdf status=%d", resp.StatusCode) }

    resp, _ = AuthRequest(t, token, "DELETE", fmt.Sprintf("http://localhost:8080/api/v1/templates/%d", emailID), nil)
    if resp.StatusCode != 204 { t.Fatalf("delete email status=%d", resp.StatusCode) }

    // Verify deleted templates return 404
    resp, _ = AuthRequest(t, token, "GET", fmt.Sprintf("http://localhost:8080/api/v1/templates/%d", emailID), nil)
    if resp.StatusCode != 404 { t.Fatalf("expected 404 for deleted email, got %d", resp.StatusCode) }

    resp, _ = AuthRequest(t, token, "GET", fmt.Sprintf("http://localhost:8080/api/v1/templates/%d", pdfID), nil)
    if resp.StatusCode != 404 { t.Fatalf("expected 404 for deleted pdf, got %d", resp.StatusCode) }

    resp, _ = AuthRequest(t, token, "GET", fmt.Sprintf("http://localhost:8080/api/v1/templates/%d", dupID), nil)
    if resp.StatusCode != 404 { t.Fatalf("expected 404 for deleted dup, got %d", resp.StatusCode) }
}
